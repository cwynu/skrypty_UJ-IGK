...
player_pick(){
  ...
  then
    play=$player_1
    echo -n "Graczu (X), wskaż pole: "
  else
    play=$player_2
    echo -n "Graczu (O), wskaż pole: "
  fi
  ...
  then 
    echo -n "To pole jest już zajęte lub jest nieprawidłowe,  wybierz inne"
    player_pick
  else
    moves[($square -1)]=$play
    ((turn=turn+1))
...
check_match() {
  ...
  if [ $game_on == false ]; then
    if [ ${moves[$1]} == 'X' ];then
      echo "Wygrywa gracz (X)"
      return 
    else
      echo "Wygrywa gracz (O)"
      return 
    fi
   fi
}
